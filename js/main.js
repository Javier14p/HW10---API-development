...
function loadPosts() {
    ...
    .then(posts => {
        ...
        posts.forEach(post => {
            ...
            postElem.innerHTML = `
            <div class="card">
                <div class="card-image"><img src="${post.imagen}" alt="Imagen del post" height="280px" style="object-fit: cover; width: -webkit-fill-available;"  data-imagen="${post.imagen}"></div>
                <div class="category"> <p style="background: cornflowerblue; color: white;" data-tipo="${post.tipo}">${post.tipo}</p> <small data-fecha="${new Date(post.fecha).toLocaleDateString()}">${new Date(post.fecha).toLocaleDateString()}</small></div>
                <div class="heading"><h2 data-titulo="${post.titulo}" contenteditable="true">${post.titulo}</h2>
                    <div class="author"> <p data-cuerpo="${post.cuerpo}">${post.cuerpo}</p></div>
                    <button id="borrarbtn" class="deletePostBtn" onclick="openDeleteModal(${post.id})">Eliminar</button>
                    <button  id="editarbtn" class="editPostBtn" data-id="${post.id}" style="color: white;">Editar</button>
                </div>
            </div>
...
document.body.addEventListener('blur', function(e) {
    if (e.target && e.target.matches('h2[data-titulo]')) {
        const newTitle = e.target.innerText;
        const postId = e.target.closest('.post').querySelector('.editPostBtn').getAttribute('data-id');
        updateTitleOnServer(postId, newTitle);
    }
}, true);
function updateTitleOnServer(postId, newTitle) {
    const data = {
        titulo: newTitle
    };
  ...
  createPostBtn.onclick = function() {
    ...
    document.body.addEventListener('click', function(e) {
        if (e.target && e.target.classList.contains('editPostBtn')) {
            ...
            const tipo = postElem.querySelector('[data-tipo]').getAttribute('data-tipo');
            const fecha = postElem.querySelector('[data-fecha]').getAttribute('data-fecha');
            const imagen = postElem.querySelector('[data-imagen]').getAttribute('data-imagen');
            
            document.getElementById('editTitulo').value = titulo;
            document.getElementById('editCuerpo').value = cuerpo;
            document.getElementById('editTipo').value = tipo;
            document.getElementById('editFecha').value = fecha;
            // Para mostrar la imagen actual, considera añadir una vista previa en tu modal y establecer su src:
