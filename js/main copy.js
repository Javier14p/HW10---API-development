...
function updateUserInterface(user) {
    let userInfoElem = document.getElementById('userInfo');  // Re-declarando aquí para estar seguro
    if (userInfoElem) { // Verificar si userInfoElem está definido
        if (user) {
            userInfoElem.textContent = user.email;
        ...
        console.error("Elemento con id 'userInfo' no encontrado");
    }
}
...
function loadPosts() {
    ...
    .then(posts => {
        ...
        posts.forEach(post => {
            ...
            postElem.innerHTML = `
            <h2 data-titulo="${post.titulo}">${post.titulo}</h2>
            <p data-cuerpo="${post.cuerpo}">${post.cuerpo}</p>
            <p data-tipo="${post.tipo}">${post.tipo}</p>
            <small data-fecha="${new Date(post.fecha).toLocaleDateString()}">${new Date(post.fecha).toLocaleDateString()}</small>
            <img src="${post.imagen}" alt="Imagen del post" width="300" data-imagen="${post.imagen}">
            <button onclick="openDeleteModal(${post.id})">Eliminar</button>
